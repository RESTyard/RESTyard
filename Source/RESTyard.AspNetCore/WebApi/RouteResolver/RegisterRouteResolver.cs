...
namespace RESTyard.AspNetCore.WebApi.RouteResolver
{
    ...
    public class RegisterRouteResolver2 : IHypermediaRouteResolver
    {
        ...
        public RegisterRouteResolver2(HttpContext httpContext, LinkGenerator linkGenerator, IRouteKeyFactory routeKeyFactory, IRouteRegister routeRegister, HypermediaExtensionsOptions hypermediaOptions, IHypermediaUrlConfig hypermediaUrlConfig)
        {
        ...
        public Result<string> RouteUrl(string routeName, object? routeKeys = null)
        {
            return linkGenerator.GetUriByName(this.httpContext, routeName, routeKeys, hypermediaUrlConfig.Scheme, hypermediaUrlConfig.Host) ?? Result<string>.Error("Could not build URL");
        }
        ...
        private ResolvedRoute GetRouteByType(Type lookupType, object? routeKeys = null)
        {
            ...
            var routeUrl = linkGenerator.GetUriByName(this.httpContext, routeInfo.Name, routeKeys, hypermediaUrlConfig.Scheme, hypermediaUrlConfig.Host);
            if (routeUrl == null)
            {
                throw new RouteResolverException($"Could not build route: '{routeInfo.Name}' with method {routeInfo.HttpMethod}");
            }
