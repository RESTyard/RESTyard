...
namespace WebApiHypermediaExtensionsCore.WebApi.AttributedRoutes
{
    public static class AttributedRouteHelper
    {
        ...
        {
            if (routeKeyProducerType != null && !typeof(IRouteKeyProducer).IsAssignableFrom(routeKeyProducerType))
            {
                throw new HypermediaRouteException(
                    $"{typeof(HttpGetHypermediaObject).Name} requires a {typeof(IRouteKeyProducer).Name} type.");
            }
        }
