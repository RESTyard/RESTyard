...
{
    [TestClass]
    public class When_serializing_to_siren
    {
        [TestMethod]
        ...
        string SerializeClassic(HypermediaObject obj)
        {
            var urlHelper = new MockUrlHelper();
            var routeRegister = new RouteRegister();
            var routeResolverFactory = new RegisterRouteResolverFactory(routeRegister, new HypermediaExtensionsOptions());
            var routeKeyFactory = new RouteKeyFactory(routeRegister);
            var queryStringBuilder = new QueryStringBuilder();
            var testUrlConfig = new HypermediaUrlConfig()
            {
                Host = new HostString("MyHost", 1234),
                Scheme = "Scheme"
            };
            var routeResolver = routeResolverFactory.CreateRouteResolver(urlHelper, routeKeyFactory, testUrlConfig);
            var sirenConverter = new SirenConverter(routeResolver, queryStringBuilder, null);

            return sirenConverter.ConvertToString(obj);
        }
    }
    ...
    public class MockUrlHelper : IUrlHelper
    {
        ...
        public string Link(string routeName, object values) => throw new System.NotImplementedException();

        public ActionContext ActionContext { get; }
    }

//    [TestClass]
//    public class GenerateSirenClasses
//    {
//        [TestMethod]
//        public async Task FromJsonSchema()
//        {
//            var json = @"{
...
//            File.WriteAllText(@"c:\temp\Siren.cs", file);
//        }
//    }
}